...
def create_input_files(dataset, karpathy_json_path, image_folder, captions_per_image, min_word_freq, output_folder,
    ...
    assert dataset in {'RSICD','LEVIR_CC','Sydney_input','UCM_input'}
    ...
    for img in data['images']:
        ...
        elif dataset == 'Sydney_input':
            # FIXME:need to change for levir_CC
            path1 = os.path.join(image_folder, 'images', img['filename'])
            path2 = os.path.join(image_folder, 'masks', img['filename'])
            path = [path1,path2]
        elif dataset == 'UCM_input':
            # FIXME:need to change for levir_CC
            path1 = os.path.join(image_folder, 'images', img['filename'])
            path2 = os.path.join(image_folder, 'masks', img['filename'])
            path = [path1,path2]
    ...
    for impaths, imcaps, split in [(train_image_paths, train_image_captions, 'TRAIN'),
        ...
        with h5py.File(os.path.join(output_folder, split + '_IMAGES_' + base_filename + '.hdf5'), 'a') as h:
            ...
            for i, path in enumerate(tqdm(impaths)):
                ...
                # Read images
                if dataset =='Sydney_input'or dataset =='UCM_input':
...
def save_checkpoint(args,data_name, epoch, epochs_since_improvement, encoder_image,sem_enhance,spatial_snhance,encoder_feat, decoder,
                    encoder_image_optimizer,sem_enhance_optimizer,spatial_enhance_optimizer,encoder_feat_optimizer, decoder_optimizer,bleu4, is_best):
    """
    ...
    state = {'epoch': epoch,
             ...
             'spatial_enhance':spatial_snhance,
