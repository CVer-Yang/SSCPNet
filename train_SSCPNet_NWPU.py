...
from model.models import CNN_Encoder
from model.SSCPNet import *
from datasets1 import *
from utils1 import *
from eval7_UCM import evaluate_transformer
data = '16_31finaNWPU'



def train(args, train_loader, encoder_image,sem_enhance,spatial_enhance,encoder_feat, decoder, criterion, encoder_image_optimizer,encoder_image_lr_scheduler,sem_enhance_optimizer,sem_enhance_lr_scheduler,spatial_enhance_optimizer,spatial_enhance_lr_scheduler, encoder_feat_optimizer,encoder_feat_lr_scheduler, decoder_optimizer, decoder_lr_scheduler, epoch):
    """
    ...
    for i, (img_pairs, caps, caplens,noms,nomlens) in enumerate(train_loader):
#         if i == 20:
#             break
        ...
        scores, caps_sorted, decode_lengths, sort_ind, scores1, noms_sorted, decode_lengths1, sort_ind1 = decoder(fused_feat, caps, noms,caplens,nomlens)


        # Since we decoded starting with <start>, the targets are all words after <start>, up to <end>
        targets = caps_sorted[:, 1:]
        targets1 = noms_sorted[:, 1:]
        ...
        loss2 = criterion(scores1, targets1)
        allloss = loss +loss2

        # Back prop.
        decoder_optimizer.zero_grad()
        encoder_feat_optimizer.zero_grad()
        sem_enhance_optimizer.zero_grad()
        spatial_enhance_optimizer.zero_grad()
        ...
        sem_enhance_lr_scheduler.step()
        spatial_enhance_lr_scheduler.step()
        if encoder_image_optimizer is not None:
...
def main(args):
    ...
    # set the encoder_dim
    encoder_image_dim = 2048 #resnet101

    if args.encoder_feat == 'CVFF':
        encoder_feat = CVFF(feature_dim=encoder_image_dim, dropout=0.5, h=14, w=14, d_model=512, n_head=args.n_heads,
                               n_layers=args.n_layers)

    sem_enhance = semantic_att(feature_dim=2048)
    spatial_enhance= spatial_att(feature_dim= 2048)

    # Decoder
    args.feature_dim_de = 512*2 # 当有concat是1024,否则为512
    ...
    sem_enhance_optimizer = torch.optim.Adam(params=filter(lambda p: p.requires_grad, sem_enhance.parameters()),
                                         lr=args.encoder_lr) if args.fine_tune_encoder else None
    sem_enhance_lr_scheduler = StepLR(sem_enhance_optimizer, step_size=900, gamma=1) if args.fine_tune_encoder else None

    spatial_enhance_optimizer = torch.optim.Adam(params=filter(lambda p: p.requires_grad, spatial_enhance.parameters()),
                                         lr=args.encoder_lr) if args.fine_tune_encoder else None
    spatial_enhance_lr_scheduler = StepLR(spatial_enhance_optimizer, step_size=900, gamma=1) if args.fine_tune_encoder else None
    ...
    # Move to GPU, if available
    ResEncoder = encoder_image.to(device)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if epochs_since_improvement > 0 and epochs_since_improvement % 5 == 0:
            ...
            if args.fine_tune_encoder and encoder_image_optimizer is not None:
                ...
                adjust_learning_rate(encoder_image_optimizer, 0.8)

        # One epoch's training
        print(time.strftime("%m-%d  %H : %M : %S", time.localtime(time.time())))
        train(args,
              train_loader=train_loader,
              encoder_image = ResEncoder,
              sem_enhance = sem_enhance,
              spatial_enhance = spatial_enhance,
              ...
              sem_enhance_optimizer=sem_enhance_optimizer,
              sem_enhance_lr_scheduler=sem_enhance_lr_scheduler,
              spatial_enhance_optimizer=spatial_enhance_optimizer,
              spatial_enhance_lr_scheduler=spatial_enhance_lr_scheduler,
        ...
        # Save checkpoint
        print(is_best)
        checkpoint_name = args.encoder_image + '_'+args.encoder_feat + '_' + args.decoder+data #_tengxun_aggregation
        save_checkpoint(args, checkpoint_name, epoch, epochs_since_improvement, encoder_image,sem_enhance,spatial_enhance,encoder_feat, decoder,
                        encoder_image_optimizer,sem_enhance_optimizer,spatial_enhance_optimizer,encoder_feat_optimizer,decoder_optimizer, metrics, is_best)
...
if __name__ == '__main__':
    ...
    # Model parameters
    parser.add_argument('--encoder_image', default="resnet50", help='which model does encoder use?')
    parser.add_argument('--encoder_feat', default='CVFF') #
    ...
    parser.add_argument('--savepath', default="./models_checkpoint/NWPU/")
